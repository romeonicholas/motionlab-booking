...
# Additional modules (located in /lib), sources noted in each module
import logging
from mfrc522 import MFRC522

# Modules created specifically for this program
import secrets
...
logging.basicConfig(filename="log_{}.txt".format(secrets.RESOURCE_ID), filemode="w")
    ...
    def __init__(self, client_id, client_secret, scope, usr, pwd):
        ...
        try:
            ...
            if request.status_code == 200:
                ...
                print(
                    "Successfully retrieved access token with scope: {}\n".format(scope)
                )

            else:
                logging.error(
                    "Failed to retrieve token, status code: {} ".format(
                        request.status_code
                    )
                )
...
def read_user_checkin_token(uid):
    ...
    # e.g. 2183925508
    user_checkin_token = str(
        int(hex(int.from_bytes(bytes(uid), "little", False))[8:], 16)
    )
...
def get_current_booking(resource_id, access_token):
    ...
    thirty_one_minutes_from_now = utime.localtime(
        utime.mktime(
            (now[0], now[1], now[2], now[3], now[4] + 31, now[5], now[6], now[7])
        )
    )
    ...
    try:
        request = urequests.get(
            "https://members.motionlab.berlin/api/resources/"
            + resource_id
        ...
        if request.status_code == 200:
            ...
            logging.error(
                "Getting resource availability failed with the following status code: {}".format(
                    request.status_code()
                )
            )
            logging.error(request.json())
...
def create_booking(membership_id, access_token, resource_id):
    ...
    thirty_minutes_from_now = utime.localtime(
        utime.mktime(
            (now[0], now[1], now[2], now[3], now[4] + 30, now[5], now[6], now[7])
        )
    )
...
def is_booking_less_than_five_minutes_old(created_at):
    now = get_now()
    five_minutes_ago = utime.localtime(
        utime.mktime(
            (now[0], now[1], now[2], now[3], now[4] - 5, now[5], now[6], now[7])
        )
    )

    booking_created_time = utime.localtime(
        utime.mktime(
            (
                created_at[0, 3],
                created_at[5, 6],
...
def end_booking(booking_id, access_token):
    ...
    try:
        request = urequests.put(
            "https://members.motionlab.berlin/api/bookings/"
            + booking_id
            + "/?access_token="
            + access_token,
            json=data,
...
def delete_booking(booking_id, access_token):
    try:
        ...
        if request.status_code == 204:
            ...
            logging.error(
                "Cannot delete bookings made by an event through this endpoint\n"
            )
    except Exception as e:
        logging.error("Error: ", e)
...
def configure_device():
    print(
        "Please answer the following questions to generate an OAUTH token for this client."
    )
    print(
        "None of your entered data will be stored locally, it will be deleted as soon as a token is generated.\n"
    )

    print(
        "Please enter the booking program's client ID (from https://www.cobot.me/oauth2_clients)"
    )
    client_id = input()
        ...
        if stat == reader.OK:
            ...
            if stat == reader.OK:
                ...
                if user_checkin_token in last_user_token_and_id:
                    ...
                    print(
                        "Membership ID was already locally available: {}\n".format(
                            membership_id
                        )
                    )
                else:
                    ...
                    print(
                        "Membership ID was not previously locally available, but will be until current booking ends\n"
                    )
                ...
                if current_booking == {}:
                    ...
                    if membership_id == current_booking["membership_id"]:
                        ...
                        if is_user_checked_in_to_booking == False:
                            ...
                            if (
                                utime.ticks_diff(time_now, onsite_booking_creation_time)
                                < TIMER_MS
                            ):
